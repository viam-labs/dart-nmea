...
String _getDynamicLibraryPath() {
  ...
  bool isCI = Platform.environment['CI'] == 'true';

  String libPath;
  if (!isCI) {
    libPath =
        path.join(Directory.current.path, 'bin', 'bin_${os}_$arch.$extension');
  } else {
    String workspaceDir = Platform.environment['GITHUB_WORKSPACE'] ?? '';
    libPath = path.join(workspaceDir, 'bin', 'bin_${os}_$arch.$extension');
  }
